...
Page({
  ...
  mathRandom: function () {
    ...
    return num > 3 ? 0 : num > 1 ? 2 : 4;
  },
  mathRandomAtt: function () {
    var t = 0, len = 4, total = 0, num = 0, att = [];
    for (; t < len; t++) {
      num = this.mathRandom();
      att[t] = num;
      total += num;
  ...
  mathRandomArr: function () {
    var t = 0,len = 4, total = 0, arr = [];
    for (; t < len; t++) {
      arr.push(this.mathRandomAtt().att);
      total += this.mathRandomAtt().total
    } 
    return total > 8 ? arr : [[0, 0, 2, 2], [0, 2, 4, 0], [0, 4, 0, 0], [0, 0, 0, 0]];
  }
})
